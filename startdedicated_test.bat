...
for /f "tokens=* skip=19" %%F in ('dir 7DaysToDieServer_Data\output_log_dedi*.txt /o-d /tc /b') do del 7DaysToDieServer_Data\%%F
...
start 7daystodieserver -logfile 7DaysToDieServer_Data\output_log_dedi%LOGTIMESTAMP%.txt -quit -batchmode -nographics -configfile=serverconfig_test.xml -dedicated
